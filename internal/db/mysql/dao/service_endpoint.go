...
func FindServiceEndpoint(ctx context.Context, exec boil.ContextExecutor, iD int64, selectCols ...string) (*ServiceEndpoint, error) {
	...
	query := fmt.Sprintf(
		"select %s from `service_endpoint` where `id`=?", sel,
	)
...
func (o *ServiceEndpoint) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if !cached {
		...
		if len(wl) != 0 {
			...
			cache.query = "INSERT INTO `service_endpoint` () VALUES ()%s%s"
		}

		var queryOutput, queryReturning string

		if len(cache.retMapping) != 0 {
			cache.retQuery = fmt.Sprintf("SELECT `%s` FROM `service_endpoint` WHERE %s", strings.Join(returnColumns, "`,`"), strmangle.WhereClause("`", "`", 0, serviceEndpointPrimaryKeyColumns))
		}
	...
	o.ID = int64(lastID)
	if lastID != 0 && len(cache.retMapping) == 1 && cache.retMapping[0] == serviceEndpointMapping["id"] {
		goto CacheNoHooks
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for service_endpoint")
	}
...
func (o ServiceEndpointSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE `service_endpoint` SET %s WHERE %s",
		...
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 0, serviceEndpointPrimaryKeyColumns, len(o)))
...
var mySQLServiceEndpointUniqueColumns = []string{
	"id",
}
...
func (o *ServiceEndpoint) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	nzDefaults := queries.NonZeroDefaultSet(serviceEndpointColumnsWithDefault, o)
	nzUniques := queries.NonZeroDefaultSet(mySQLServiceEndpointUniqueColumns, o)
	...
	if !cached {
		...
		cache.query = buildUpsertQueryMySQL(dialect, "service_endpoint", update, insert)
		cache.retQuery = fmt.Sprintf(
			"SELECT %s FROM `service_endpoint` WHERE %s",
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for service_endpoint")
	}
	...
	uniqueMap, err = queries.BindMapping(serviceEndpointType, serviceEndpointMapping, nzUniques)
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for service_endpoint")
	}
...
func (o *ServiceEndpoint) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `service_endpoint` WHERE `id`=?"
...
func ServiceEndpointExists(ctx context.Context, exec boil.ContextExecutor, iD int64) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `service_endpoint` where `id`=? limit 1)"
