...
ui/run:
	yarn --cwd $(UI_ROOT_DIR) start

.PHONY: service/run
## Run the DB (Postgres) and HTTP server (will wipe local DB)
service/run: db/postgres/run service/postgres/run

.PHONY: service/postgres/run
...
db/postgres/run:
	...
	@$(MAKE) db/postgres/await

.PHONY: db/postgres/await
